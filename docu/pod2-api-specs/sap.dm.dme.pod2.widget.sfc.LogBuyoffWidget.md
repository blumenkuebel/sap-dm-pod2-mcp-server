...
`sap.dm.dme.pod2.widget.sfc.LogBuyoffWidget`

Shows the log buyoff entries for the selected SFC and operations and allows accepting or rejecting buyoffs.

Buyoffs can be configured in the Manage Buyoffs app. There the user can create a buyoff which makes an operation
activity require an extra step of approval (buyoff) before the operation activity can be completed. The buyoff
is configured with an assignment which matches by material, routing, operation activity, work center, resource,
or order. The buyoff also needs to be configured with supervisor users who can accept or reject the buyoff.

When a user completes an operation activity which has a buyoff assigned, the operation activity goes into the
Complete Pending status and there is a pending buyoff log entry created. This widget displays those pending
buyoff log entries for the selected SFC and operations. One of the supervisor users can either accept or reject
the buyoff from this widget. Accepting the buyoff completes the operation activity, while rejecting the buyoff
puts it back into the "In Queue Rework" status.
...
### _oCommentTextArea :sap.m.TextArea
...
Creates the table to display the buyoff logs.
...
Creates the header toolbar for the table.
...
### _createView () → {sap.ui.core.Element}

Creates the main view of the widget.
...
### _getFindBuyoffLogsRequest () → {sap.dm.dme.pod2.api.internal.sfc.FindBuyoffLogsRequest}

Gets the find buyoff logs request based on the selected SFCs and operation activities.

🔧 This method may be overridden by custom subclasses.

🔧 This method may be overridden by custom subclasses.

**Returns:** sap.dm.dme.pod2.api.internal.sfc.FindBuyoffLogsRequest - [sap.dm.dme.pod2.api.internal.sfc.FindBuyoffLogsRequest](sap.dm.dme.pod2.api.internal.sfc.md#.FindBuyoffLogsRequest)

### _getItemTemplate () → {sap.m.ColumnListItem}

Gets the template for each item (row) in the table.
...
### _getLogBuyoffsRequest (sAction, sComment, aBuyoffLogs) → {sap.dm.dme.pod2.api.internal.sfc.LogBuyoffsRequest|null}

Constructs the log buyoffs request based on the selected buyoff logs in the table.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `sAction` | sap.dm.dme.pod2.api.internal.sfc.SfcInternalApiClient.BuyoffLogAction | No |  |
| `sComment` | string | No |  |
| `aBuyoffLogs` | Array.<sap.dm.dme.pod2.api.internal.sfc.BuyoffLog> | No |  |

**Returns:** sap.dm.dme.pod2.api.internal.sfc.LogBuyoffsRequest|null - [sap.dm.dme.pod2.api.internal.sfc.LogBuyoffsRequest](sap.dm.dme.pod2.api.internal.sfc.md#.LogBuyoffsRequest) |  null

### _getTableColumns () → {Array.<sap.m.Column>}
...
### (async) _logBuyoffs (sAction) → {Promise.<void>}

Logs the selected buyoffs with the specified action.
...
### _onLogBuyoffsError (oError)

Handles log buyoffs API call error.
...
### _onLogBuyoffsSuccess (oRequest)

Handles successful log buyoffs API call.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `oRequest` | sap.dm.dme.pod2.api.internal.sfc.LogBuyoffsRequest | No |  |

### _onRejectButtonPress ()

Handles the Reject button press event.

🔧 This method may be overridden by custom subclasses.

🔧 This method may be overridden by custom subclasses.

### (async) _refreshBuyoffs () → {Promise.<void>}

Refreshes the buyoff logs based on the selected SFCs and operation activities.
