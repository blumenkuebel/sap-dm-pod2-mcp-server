# NavigateToWidgetAction

`sap.dm.dme.pod2.action.page.NavigateToWidgetAction`

Action to navigate to a specific widget. This action not only navigates to a different page if necessary, but
also sets any selected IconTabBar tabs such that the target widget is visible.
...
```
new NavigateToWidgetAction (oActionConfig)
```
