...
```
new WorkListTableWidget (oConfig)
```
...
### PROPERTY_CATEGORY_OVERRIDE :Object.<string, string>
...
Override parent's method to add highlight to the row if it's in error state and synchronize selected items
with the PodContext's selected work list items.
...
Use the current work list sorting information from the POD Context.
...
### (async, static) getCustomFields () → {Promise.<Array.<sap.dm.dme.pod2.widget.core.TableWidget.ColumnConfig>>}
