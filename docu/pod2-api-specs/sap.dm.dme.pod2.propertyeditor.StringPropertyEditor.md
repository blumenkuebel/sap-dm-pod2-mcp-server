...
```
new StringPropertyEditor (oPropertyAccessor, sProperty, vSettingsopt)
```
...
| `vSettings` | sap.m.InputType | sap.dm.dme.pod2.control.$CustomInputSettings | Yes | A settings object for the Input control, or the input type (for backward compatibility). |
