...
Property editor for CSS size properties (e.g., "16px", "1.5em", "100%"). Shows a text box for the value and a
select box for the unit.

Note: Only the most common units px, em, rem, and % are supported. If no unit is selected, the value is treated
as a raw string and the user can use less common units or use "calc(...)".
...
```
new CssSizePropertyEditor (oPropertyAccessor, sProperty)
```
