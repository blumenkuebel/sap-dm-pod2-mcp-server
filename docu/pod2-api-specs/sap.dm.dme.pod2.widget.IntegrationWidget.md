...
The base class for widgets that integrate with a third-party library or UI framework.

Widgets created with SAPUI5 should extend [sap.dm.dme.pod2.widget.Widget](sap.dm.dme.pod2.widget.Widget.md) instead.
...
### constructor :TypeOf.<sap.dm.dme.pod2.widget.IntegrationWidget>
...
### (async) _createView () → {Promise.<sap.ui.core.Control>}

Creates an HTML container for the third-party UI component to mount to, and invokes the entrypoint indicated
by [sap.dm.dme.pod2.widget.IntegrationWidget.getEntryPoint](sap.dm.dme.pod2.widget.IntegrationWidget.md#.getEntryPoint). The arguments passed to the entrypoint
function are the DOM element to mount to, and the Widget instance.

🔧 This method may be overridden by custom subclasses.

🔧 This method may be overridden by custom subclasses.

**Returns:** Promise.<sap.ui.core.Control> - Promise.<sap.ui.core.Control>
...
Coalseces the default height/width values if the config contains falsey values.
...
### (abstract) (abstract, static) getEntryPoint () → {string|Promise.<string>}

Returns an SAPUI5 module path which maps to the entrypoint for your third-party UI component. The entrypoint
must use sap.ui.define and return a function which accepts a DOM element and the Widget instance as
arguments. Refer to the POD Developer's Guide and sample extension for more information about the
expectations of your component's entry point.

This function may be asynchronous, for example if the entrypoint must be retrieved from a manifest file.
...
### IntegrationWidgetEntryPoint (oDomRef, oWidget)

**Parameters:**

| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `oDomRef` | HTMLDivElement | No | The DOM element to mount the UI component to |
| `oWidget` | sap.dm.dme.pod2.widget.IntegrationWidget | No | The widget instance |
