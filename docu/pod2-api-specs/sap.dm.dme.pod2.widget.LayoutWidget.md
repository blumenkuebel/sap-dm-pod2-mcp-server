...
Base class for metadata-driven layout widgets that wrap SAPUI5 layout controls that can contain other controls.
This class will automatically implement all necessary methods.

The subclass must define the standard static widget methods.

The subclass' constructor must call the super constructor with the control class and configuration object.

Subclasses can define the following properties in the class to include or exclude aggregations:

INCLUDE_AGGREGATIONS: Array of aggregation names to include. If not defined, all aggregations are included.
EXCLUDE_AGGREGATIONS: Array of aggregation names to exclude. If not defined, no aggregations are excluded.
DEFAULT_AGGREGATION: The name of the default aggregation. This is the aggregation that will be used when
moving another widget to this widget without specifying an aggregation. If not defined, the first aggregation
returned by getAggregations() will be used.
...
|------|------|----------|-------------|
| `_aAggregations` | Array.<sap.dm.dme.pod2.widget.metadata.WidgetAggregation> \| undefined | Yes | @static |
...
### (static) DEFAULT_AGGREGATION :string|undefined

The default aggregation is the aggregation that will be used when adding a widget without specifying an
aggregation. If not defined, the first aggregation returned by getAggregations() will be used.

### (static) EXCLUDE_AGGREGATIONS :Array.<string>

List of aggregation names to exclude. If not defined, no aggregations are excluded.
...
excluded from the POD Designer. If not defined, no properties are excluded.

### (static) INCLUDE_AGGREGATIONS :Array.<string>

List of aggregation names to include. If not defined, all aggregations are included.
...
### constructor :TypeOf.<sap.dm.dme.pod2.widget.LayoutWidget>

## Methods

### _addWidgetToView (oAggregation, oWidget, iIndex)

Adds a widget's view to the given aggregation. This only affects the underlying controls and has no effect
on the POD hierarchy.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `oAggregation` | sap.dm.dme.pod2.widget.metadata.WidgetAggregation | No |  |
...
Removes a widget's view from the given aggregation. This only affects the underlying controls and has no
effect on the POD hierarchy.
...
### getAggregations () → {Array.<sap.dm.dme.pod2.widget.metadata.WidgetAggregation>}
...
**Returns:** Array.<sap.dm.dme.pod2.widget.metadata.WidgetAggregation> - Array.<[sap.dm.dme.pod2.widget.metadata.WidgetAggregation](sap.dm.dme.pod2.widget.metadata.WidgetAggregation.md)>
