# OrderListTableWidget

`sap.dm.dme.pod2.widget.worklist.OrderListTableWidget`

You use this widget to view a list of process orders filtered based on certain criteria. You can view a list of
process orders. With certain design time configuration, user can navigate to order detail page.
...
Overrides the parent's _createTable method to create the header toolbar and pass in in default mSettings for
the widget.
...
|------|------|----------|-------------|
| `oConfig` | sap.dm.dme.pod2.widget.WidgetConfig | No | The widget configuration |
| `mSettings` | sap.m.$TableSettings | No | The settings for the table |
...
### _createToolbarTitle () → {sap.ui.core.Control}

Create the toolbar title for the work list table.
...
Handles when an item in the work list table is pressed. This method sets the selected work list items in
PodContext.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `oEvent` | sap.m.ListBase$ItemPressEvent | No | The SAPUI5 event object. |

### _onPrintMenuItemPress (sPrintMode)

Handles the print action for selected items in the work list table.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `sPrintMode` | sap.dm.dme.pod2.dialog.PrintDocumentDialog.PrintMode | No |  |

### _onSelectionChange (oEvent)

Handles when the selection in the work list table changes. This method updates the selected work list items
in PodContext.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `oEvent` | sap.m.ListBase$SelectionChangeEvent | No | The SAPUI5 event object. |

### _onSort (aSorting)

This function is called whenever the sorting criteria changes for the widget, if applicable.

This does not include the initial load. The initial sort order should be checked during `onInit`
if necessary.
...
Selects the table list items based on the currently selected work list items in the PodContext.
...
Overrides the parent method to coalias default values.
