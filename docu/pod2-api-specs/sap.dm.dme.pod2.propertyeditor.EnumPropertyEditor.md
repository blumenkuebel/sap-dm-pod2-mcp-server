...
Represents a special case of SelectPropertyEditor in which the key and value are reversed.

For example, in the case of the simple "log level" enum below, the object key is the friendly text
associated with each option, while the value is the meaningful identifier that should be stored in the
widget's configuration. In this case we want to translate and display the text for "ERROR" on the Select item,
not the number `1`.

**Extends:** [sap.dm.dme.pod2.propertyeditor.SelectPropertyEditor](sap.dm.dme.pod2.propertyeditor.SelectPropertyEditor.md)
...
```
new EnumPropertyEditor (oPropertyAccessor, sProperty, vItems, sDefaultValueopt)
```
...
| `sProperty` | string | No |  |
| `vItems` | sap.ui.base.DataType | Object.<string, string> | Array.<string> | No | The items to show in the Select.  In addition to the types passed to SelectPropertyEditor, oItems can also be a registered enum of type `sap.ui.base.DataType`. If a registered enum is used, translation will be attempted using the PodContext i18n model for both the label and enumeration items, overriding the displayName and description strings provided to the constructor of the corresponding Property.  See https://sapui5.hana.ondemand.com/sdk/#/api/sap.ui.base.DataType for more information. |
| `sDefaultValue` | string | Yes |  |
