...
This base widget is responsible for creating the view for the table widget. This class can be subclassed by
custom widgets to change the behavior of the table widget.

Table widgets should implement the static `getFields` function to return the default list of column
metadata.

The configuration for a table widget contains two special properties; `columns` and, optionally,
`defaultSorting`.

**Extends:** [sap.dm.dme.pod2.widget.ControlWidget](sap.dm.dme.pod2.widget.ControlWidget.md)

## Constructor

```
new TableWidget (oConfig)
```
...
|------|------|----------|-------------|
| `oConfig` | sap.dm.dme.pod2.widget.WidgetConfig | No |  |
...
Additional widget properties beyond those derived from sap.m.Table.
...
### (abstract) (abstract) _createCell (oColumnConfig) → {sap.ui.core.Control}

Given a column configuration, create the control to show in the cell of the table for that column. This
method can be overridden in subclasses to provide custom cell controls.
...
### (abstract) (abstract) _getModelPath () → {string}

Gets the model path that the table's `items` aggregation should be bound to.
...
### _getWidgetPropertyEditor (oProperty) → {sap.dm.dme.pod2.propertyeditor.PropertyEditor}

Override the base class method to customize the property editor for specific properties.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `oProperty` | sap.ui.base.ManagedObject.MetadataOptions.ext.Property | No |  |
...
### (static) getCustomFields () → {Promise.<Array.<sap.dm.dme.pod2.widget.core.TableWidget.ColumnConfig>>}

Optional method to be implemented by subclasses to return any custom fields that should be added to the
table.

Can use the _getCustomFieldsForTable() utility method to retrieve custom field definitions.

🔧 This method may be overridden by custom subclasses.

🔧 This method may be overridden by custom subclasses.

**Returns:** Promise.<Array.<sap.dm.dme.pod2.widget.core.TableWidget.ColumnConfig>> - Promise.<Array.<[sap.dm.dme.pod2.widget.core.TableWidget.ColumnConfig](sap.dm.dme.pod2.widget.core.TableWidget.md#.ColumnConfig)>>

### (static) getDefaultConfig () → {sap.dm.dme.pod2.widget.core.TableWidget.DefaultConfig}

Returns a default config reflecting the default state of the table's columns as returned by
`getFields()`, along with common defaults suitable for most tables.

Default properties are as follows and can be overridden by the subclass as needed.

{
   properties: {
       autoPopinMode: true,
       busyIndicatorDelay: 0,
       popinLayout: PopinLayout.GridSmall,
...
static getDefaultConfig() {   return {       properties: {           ...super.getDefaultConfig().properties,           growingScrollToLoad: true,           mode: ListMode.SingleSelectMaster,           pageSize: 100,           sorting: Tuple<
               {                   sortBy: WorkListTableWidget.Field.SFC,                   descending: false               }           >
       }   };}
```

### (static) getDefaultFields () → {Array.<string>}

Optional method to be implemented by subclasses to return the default fields to be shown in the table when
the widget is created for the first time. This can be a subset of the field names returned by
`getFields()`. If not implemented, all fields returned by `getFields()` will be shown
by default.
...
Returns the default field metadata for the objects in the table. This can be used to build the default
configuration and indicate which fields are sortable.

It is important to note that new columns added to a widget will not exist as part of existing POD
configurations.
...
The configuration for each column of the table, with some added metadata.

### ConfigProperties ()

### DefaultConfig ()

The default configuration object for a TableWidget, including important special properties for customizing
the table columns and sorting behavior.

### QuantityBulletChartBindings ()

### Sorting ()

A plain-object equivalent to [sap.dm.dme.pod2.context.type.Sorting](sap.dm.dme.pod2.context.type.Sorting.md) as it is persisted in the widget
configuration.
