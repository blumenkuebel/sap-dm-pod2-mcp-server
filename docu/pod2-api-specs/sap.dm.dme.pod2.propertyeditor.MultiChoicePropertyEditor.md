# MultiChoicePropertyEditor

`sap.dm.dme.pod2.propertyeditor.MultiChoicePropertyEditor`

A property editor for editing a property where multiple values can be selected from a list of choices. Uses a
...
```
new MultiChoicePropertyEditor (oPropertyAccessor, sProperty, vItems, aDefaultValuesopt)
```
...
| `vItems` | object | Array.<string> | sap.ui.base.DataType | Array.<sap.ui.core.$ListItemSettings> | No | The items to display in the MultiComboBox:  If an object is provided then for each property in the object the property name is used as the text and the property value is used as the key. If an array of strings is provided then the string is used as both the key and the text. If an sap.ui.base.DataType is provided then the enum values of the DataType are used as the items. If an array of ListItemSettings is provided then each Item is created using the provided settings. |
| `aDefaultValues` | Array.<string> | Yes | The default values to use if the property value is not set. The list is the list of keys that should be selected by default. |
