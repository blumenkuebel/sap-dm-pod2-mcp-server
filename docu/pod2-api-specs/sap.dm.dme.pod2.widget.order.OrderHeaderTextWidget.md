# OrderHeaderTextWidget

`sap.dm.dme.pod2.widget.order.OrderHeaderTextWidget`

A widget to display order header text.
...
### (async) _getOrderHeaderText (sOrderId) → {Promise.<string>}

Gets the order header text.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `sOrderId` | string | No | The order id |
