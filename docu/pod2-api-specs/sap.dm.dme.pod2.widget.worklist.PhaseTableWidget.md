...
`sap.dm.dme.pod2.widget.worklist.PhaseTableWidget`
...
### (async) _completePhase (oEvent) → {Promise.<void>}
...
Override parent's method to adjust the terminology of the ID column based on the order type.
...
| `mSettings` | sap.m.$TableSettings | Yes | Settings overrides for the table constructor. |
...
Create the toolbar title for the phase list table. The label is based on whether the list is displaying
phases for process orders or production orders.
...
### _getGroupHeader (oGroup) → {sap.m.GroupHeaderListItem}

Creates a group header for the table.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `text` | string | No | Text to show in the group header. |

**Returns:** sap.m.GroupHeaderListItem - sap.m.GroupHeaderListItem
...
Handles when an item in the operation table is selected. This method updates the selected operations in
PodContext.
...
### (async) _startPhase (oEvent) → {Promise.<void>}
...
Gets the default column configurations for the table.
