...
`sap.dm.dme.pod2.action.sfc.dialog.RoutingDialog`

Base class for MultipleRoutingDialog and NCRoutingDialog containing common logic.

## Constructor

```
new RoutingDialog (oCompleteSfcRequest, aNextStepOptions)
```
...
|------|------|----------|-------------|
| `oCompleteSfcRequest` | sap.dm.dme.pod2.api.execution.CompleteSfcRequest | No |  |
| `aNextStepOptions` | Array.<sap.dm.dme.pod2.api.execution.NextStepOption> | No |  |
...
### (abstract) (abstract) _createCells () → {Array.<sap.ui.core.Control>}

Creates the cells for each column in the table. This method is to be implemented by subclasses.
...
### (abstract) (abstract) _createColumns () → {Array.<sap.m.Column>}

Creates the columns for the table. This method is to be implemented by subclasses.
