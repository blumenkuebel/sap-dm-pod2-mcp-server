...
`sap.dm.dme.pod2.action.core.FocusElementAction`

Action that shifts focus to a specific control. This is primarily useful for moving the cursor to inputs which
are expected to be used with barcode scanners or similar devices to enable hands-off filling of forms.

If the provided ID matches a focusable HTML element, that element will receive focus. If the matching element is
not focusable, the first focusable child element (input, button, link, etc.) will receive focus.

For example, using the ID of a WorkListFilterBarWidget will result in the first (leftmost) filter receiving focus
because the filter bar control uses the widget's ID, but is not focusable on its own.
...
### getFocusableElement (sTargetId) → {Element|void}

Returns the nearest focusable element for a given ID. This is either the HTML element with the same ID, or
its first matching focusable child.
...
**Returns:** Element|void - Element |  void
