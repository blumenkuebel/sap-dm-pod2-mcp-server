...
`sap.dm.dme.pod2.widget.order.MaterialImageWidget`

A widget to display the material image.

**Extends:** [sap.dm.dme.pod2.widget.core.ImageWidget](sap.dm.dme.pod2.widget.core.ImageWidget.md)

## Constructor

```
new MaterialImageWidget ()
```
...
### (static) PLACEHOLDER_IMAGE_URL :string
...
### (async) _getMaterialImageUrl (oWorkListItem) → {Promise.<string>}

Gets the URL to download the material image. Returns null if no file is found for the material or if
there is an error retrieving the file id.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `oWorkListItem` | sap.dm.dme.pod2.context.type.BaseWorkListItem | No | The selected work list item |
...
### (async) _updateImage () → {Promise.<void>}

Updates the image based on the currently selected work list item.
