...
Widget properties, not including those templated for each filter field.

### UrlParameter :Enum.<string>

URL Parameters used by the widget. The keys match the widget's filter names.

### constructor :TypeOf.<sap.dm.dme.pod2.widget.worklist.WorkListFilterBarWidget>
...
### _createFilterControl (sFilterName) → {sap.ui.core.Control}

Creates the control for the given filter to be placed in the FilterGroupItem.

Called once per filter name returned by `_getFilterNames`.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `sFilterName` | string | No | The name of the filter. |

**Returns:** sap.ui.core.Control - sap.ui.core.Control

### _createFilterGroupItem (sFilterName) → {sap.ui.comp.filterbar.FilterGroupItem}
...
**Returns:** sap.ui.comp.filterbar.FilterGroupItem - sap.ui.comp.filterbar.FilterGroupItem

### _createView () → {sap.ui.core.Control}
...
### _getFilterControl (sFilterName) → {sap.ui.core.Control}

Gets the input control for the given filter.
...
### _getFilterGroupItem (sFilterName) → {sap.ui.comp.filterbar.FilterGroupItem}

Gets the filter group item for the given filter.
...
### _getInitialFilterValue (sFilterName) → {string}

Returns the default/initial value for a given filter. This prioritizes an already-set POD Context value,
then a URL parameter if enabled, and finally uses the configuration literal as a fallback.
...
### (async) _onAutoSearch () → {Promise.<void>}

Event handler for auto-submit of the filters. This differs from clicking the "Go" button in that errors
will return silently (since the user may have just not finished entering all required fields) and equivalent
filters will not ignore the minimum refresh interval.
...
Event handler for the Clear button click on the FilterBar. This will clear all the filter values.
...
Event handler for the Go button click on the FilterBar.
...
### _updatePodContext () → {sap.dm.dme.pod2.context.WorkListFilter}

Called by both _onSearch and _onClear to update the PodContext with the current filter values.
...
Called by both _onSearch and _onClear to update the URL parameters with the current filter values if
URL parameters are enabled.
...
Check for invalid or missing mandatory fields. If an error is present, an Error is thrown with a
...
### (async) _waitForPendingFilters () → {Promise.<void>}

Resolves when all self-validating filters (work center, resource, etc.) have finished async validation.
...
### getProperties () → {Array.<sap.dm.dme.pod2.widget.metadata.WidgetProperty>}

🔧 This method may be overridden by custom subclasses.

🔧 This method may be overridden by custom subclasses.

**Returns:** Array.<sap.dm.dme.pod2.widget.metadata.WidgetProperty> - Array.<[sap.dm.dme.pod2.widget.metadata.WidgetProperty](sap.dm.dme.pod2.widget.metadata.WidgetProperty.md)>
...
Override the setProperty to handle any properties that need to update the control.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `sName` | string | No | The name of the property |
| `vValue` | any | No | The value of the property |

### (static) _getFilterNames () → {Array.<string>}

Returns the list of filter names used to build the filter bar. By overriding this function, filters
can be added or removed. Adding new filters may require other functions to be overridden to handle
the new filter, such as `_createFilterControl` and `onSearch`, especially if the
new filter is not a simple text input.

The default filters are handled as follows:

sfc - Single-select with value help (stored in the model as a string)
