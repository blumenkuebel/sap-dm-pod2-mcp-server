...
`sap.dm.dme.pod2.widget.sfc.OrderScheduleWidget`

Shows the order schedule for the selected SFC and operations.
...
### _createListItemTemplate () → {sap.m.ColumnListItem}

Creates a template for the rows in the schedule table.
...
### _createScheduleTable () → {sap.ui.core.Control}

Creates the table control that displays the order schedules.
...
### _createSummary () → {sap.ui.core.Control}
...
### (async) _refreshOrderSchedule ()

Makes an API call to refresh the order schedule data and set it into the view model.
