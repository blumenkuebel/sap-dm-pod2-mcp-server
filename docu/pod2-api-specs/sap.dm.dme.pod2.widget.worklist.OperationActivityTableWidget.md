# OperationActivityTableWidget

`sap.dm.dme.pod2.widget.worklist.OperationActivityTableWidget`
...
```
new OperationActivityTableWidget ()
```
...
Column field identifiers for the operation activity table.
...
### _createActionCell () → {sap.m.HBox}

Creates the action button cell with Start/Complete buttons.

🔧 This method may be overridden by custom subclasses.

🔧 This method may be overridden by custom subclasses.

**Returns:** sap.m.HBox - sap.m.HBox

### _createCell (oColumnConfig) → {sap.ui.core.Control}

Creates the appropriate cell control for each column type.
...
### _createTable (oConfig) → {sap.m.Table}
...
Returns the default binding info with optional resource filtering.
...
| `oTemplate` | sap.m.ListItemBase | No | The list item template. |
...
### (async) _onCompleteButtonPress (oEvent) → {Promise.<void>}

Completes an operation activity.
...
### (async) _onStartButtonPress (oEvent) → {Promise.<void>}

Starts an operation activity.
...
### _onTableSelectionChange (oEvent)

Handles when an item in the operation table is selected. This method updates
the selected operations in PodContext.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `oEvent` | sap.m.ListBase$SelectionChangeEvent | No | The SAPUI5 press event object. |
...
Selects the table list items based on the currently selected operation activities in the PodContext.
...
Returns the default fields to be shown when the widget is created for the first time.
...
Gets all available column configurations for the operation activity table.
