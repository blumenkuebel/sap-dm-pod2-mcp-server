...
### (abstract) (abstract) _createView () → {sap.ui.core.Element|Promise.<sap.ui.core.Element>}

Invoked by the framework to create the view for the widget. This method must be overridden by subclasses.

🔧 This method may be overridden by custom subclasses.

🔧 This method may be overridden by custom subclasses.

**Returns:** sap.ui.core.Element|Promise.<sap.ui.core.Element> - sap.ui.core.Element |  Promise.<sap.ui.core.Element>
...
### getProperties () → {Array.<sap.dm.dme.pod2.widget.metadata.WidgetProperty>|null}

Gets the list of properties that can be configured for the widget. This method should be overridden by
subclasses.
...
**Returns:** Array.<sap.dm.dme.pod2.widget.metadata.WidgetProperty>|null - Array.<[sap.dm.dme.pod2.widget.metadata.WidgetProperty](sap.dm.dme.pod2.widget.metadata.WidgetProperty.md)> |  null
...
Overridable method to initialize the widget. This method is called after the view is created.
...
Generates a unique ID for a widget based on the widget type.
...
Generates multiple unique IDs for a widget based on the widget type.
...
|------|------|----------|-------------|
| `sWidgetType` | string | No | The widget type. |
| `iCount` | number | No | The number of unique IDs to generate. |

**Returns:** Array.<string> - Array.<string>

### (static) getCategory () → {string}

The category of the widget. The category is used to group related widgets in the widget palette of the POD
Designer together.

If not overridden, the widget will be placed in the "Uncategorized" category.
...
### (static) getDefaultConfig (sId) → {sap.dm.dme.pod2.widget.DefaultWidgetConfig}

Optional method to be implemented by subclasses that gets the default configuration for the widget when a
new widget is created in POD Designer. These default properties are persisted to the POD configuration.

In some cases, these defaults are temporary values expected to be changed by the user. For example, the
default configuration for a Text widget includes placeholder text so that the widget is visible and can be
selected with the mouse in the Preview Panel, rather than being initially invisible due to having no text
content. Similarly, a layout widget may be given an initial height or width so it has some initial minimum
size and can have new widgets dragged onto it.

The other primary use case for a defaultConfig is to provide a starting point for configuration properties
which a user would not expect to change retroactively. For example, it is generally expected that a
ScrollContainer widget will be used to add vertical scrolling to a widget, so the defaultConfig enables and
persists this behavior. Similarly, setting a default ListMode property would be appropriate because even if
the default mode changes in the future, users would not expect this new default to retroactively apply to
existing instances of the widget.

The defaultConfig object is not suitable for situations where:

The default value for a property may change in the future, and,
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `sId` | string | No | A generated ID for the widget. The provided ID can be used to build semantic IDs for default child widgets, or can be overridden by specifying a different ID in the returned object. |
...
Gets the icon to show in the widget palette of the POD Designer.

This is an optional override and will return "sap-icon://question-mark" if not implemented.
...
**See also:** [https://openui5.hana.ondemand.com/test-resources/sap/m/demokit/iconExplorer/webapp/index.html#/overview](https://openui5.hana.ondemand.com/test-resources/sap/m/demokit/iconExplorer/webapp/index.md#/overview)

### (static) getLayoutDataClass (sAggregationId) → {TypeOf.<sap.ui.core.LayoutData>|null}

For widgets that can contain child widgets, the child may need to provide layout data to configure how to
layout the child within the parent widget. The layout data class that's used depends on the underlying
SAPUI5 control used by the parent widget. For example a Splitter control requires a SplitterLayoutData
object. Each aggregation may require a different layout data class.

This method should be implemented by subclasses if they require support for layout data.
...
| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `sAggregationId` | string | No | The ID of the aggregation to get the layout data class for. |

**Returns:** TypeOf.<sap.ui.core.LayoutData>|null - TypeOf.<sap.ui.core.LayoutData> |  null

### (static) getStyleSheet () → {string|null}

If overridden to return a path to a CSS file, the stylesheet will be loaded when the widget is created.

Custom styles should always be prefixed with a unique class name to avoid affecting other components.
...
Event object passed in the WidgetPropertyChangeListener callback.

### WidgetPropertyChangeListener (oEvent)

Callback to get notified when a widget property is changed.

**Parameters:**

| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `oEvent` | sap.dm.dme.pod2.widget.Widget.WidgetPropertyChangeEvent | No | The event object. |

### findWidgetComparator (oWidget) → {boolean}

Callback for the findWidget method that is used to search the widget hierarchy.

**Parameters:**

| Name | Type | Optional | Description |
|------|------|----------|-------------|
| `oWidget` | sap.dm.dme.pod2.widget.Widget | No | The widget that is being processed. |

**Returns:** boolean - boolean

### forEachWidgetCallback (oWidget)

Callback for the forEachWidget method that is called for each widget in the hierarchy.
