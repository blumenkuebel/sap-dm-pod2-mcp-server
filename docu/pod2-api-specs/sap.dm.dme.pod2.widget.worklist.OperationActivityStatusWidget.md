# OperationActivityStatusWidget

`sap.dm.dme.pod2.widget.worklist.OperationActivityStatusWidget`

A widget that displays the status of a selected SFC in the worklist with an icon and color representing the
SFC's current status.
...
```
new OperationActivityStatusWidget ()
```
...
An enumeration of widget property IDs supported by this widget.
...
Override to update design-time preview.
